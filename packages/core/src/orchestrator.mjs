// packages/core/src/orchestrator.mjs                                                        // 오케스트레이터 본체
// Part 1/2: 개요·의존성·컨텍스트·LLM/Agent 라우팅(독립 프리셋)·프롬프트 생성                 // 파트 개요

import { readFileSync, writeFileSync, existsSync } from "node:fs";                         // 파일 IO 유틸
import { execSync } from "node:child_process";                                             // 외부 명령 실행
import path from "node:path";                                                              // 경로 유틸
import fs from "node:fs";                                                                  // 파일시스템
import process from "node:process";                                                        // 프로세스/ENV
import { loadPlugins } from "./tokens/registry.mjs";                                       // /auto 토큰 로더
import { pickLLMAndAgent } from "./router.mjs";                                            // 라우터(독립 프리셋)
import { makeOpenAI, runOpenAI } from "./llm/openai.mjs";                                  // OpenAI 어댑터
import { makeGemini, runGemini } from "./llm/gemini.mjs";                                  // Gemini 어댑터
import { runWithClaude } from "./agents/claude.mjs";                                       // Claude 실행기
import { runWithCursor } from "./agents/cursor.mjs";                                       // Cursor 실행기

function nowIso(){ return new Date().toISOString(); }                                      // ISO 타임스탬프 유틸

export async function runOrchestrator({ repoRoot, configPath, eventPath }) {               // 엔트리 포인트
  const cfg = JSON.parse(readFileSync(path.join(repoRoot, configPath), "utf8"));           // 설정 로드
  const tools = cfg.tools || {};                                                           // 도구 설정
  const policy = cfg.policy || {};                                                         // 정책 설정
  const labelsCfg = cfg.labels || {};                                                      // 라벨 키
  const pluginsPaths = cfg.plugins || [];                                                  // 플러그인 경로 목록
  const pipeline = cfg.pipeline || { commands: {} };                                       // 파이프라인 훅

  let evt = {};                                                                            // 이벤트 객체 초기화
  try { evt = JSON.parse(readFileSync(eventPath, "utf8")); } catch {}                      // 이벤트 로드(실패 허용)

  const eventName = process.env.GITHUB_EVENT_NAME || "";                                   // 이벤트명
  const isIssue = !!evt.issue && !evt.comment;                                             // 이슈 이벤트 여부
  const isDispatch = eventName === "workflow_dispatch";                                    // 디스패치 여부

  const dispatchBody = (evt?.inputs?.body ?? process.env.AUTO_INPUT_BODY ?? "");           // 디스패치 본문(입력)
  const issueBody    = (evt?.issue?.body ?? "");                                           // 이슈 본문
  const commentBody  = (evt?.comment?.body ?? "");                                         // 댓글 본문
  const rawBody = (isDispatch ? dispatchBody : (isIssue ? issueBody : commentBody)).trim();// 최종 본문(트림)

  const labels = (evt.issue?.labels || []).map(l => l?.name || "");                        // 라벨 목록
  const sourceIssueNumber = evt?.issue?.number || null;                                    // 원 이슈 번호(있으면)

  if (!rawBody || !rawBody.startsWith("/auto")) return { skipped: true };                  // /auto 가드(미존재 시 종료)

  if (!isDispatch) {                                                                       // 디스패치 외 이벤트인 경우
    if (!labels.includes(labelsCfg.run || "automation:run")) return { skipped: true };    // 필수 라벨 미존재 시 종료
  }

  const outDir = process.env.AUTO_OUT_DIR                                                  // 산출물 디렉터리
    ? path.resolve(process.env.AUTO_OUT_DIR)                                               // ENV 경로 우선
    : path.join(repoRoot, ".github", "auto");                                              // 기본 경로
  fs.mkdirSync(outDir, { recursive: true });                                               // 경로 생성 보장
  const stageLogPath = path.join(outDir, "stage-log.json");                                // stage 로그 경로
  const runMetaPath  = path.join(outDir, "run-meta.json");                                 // run 메타 경로

  const RUN_ID = Date.now();                                                               // 실행 식별자(에포크 ms)
  const SERVICE_ROOT = process.env.AUTO_SERVICE_ROOT                                       // 서비스 루트(샌드박스)
    ? path.resolve(repoRoot, process.env.AUTO_SERVICE_ROOT)                                // ENV 지정 시 사용
    : path.join(repoRoot, "apps", `auto-${RUN_ID}`);                                       // 기본 apps/auto-<id>
  const REL_SERVICE_ROOT = path.relative(repoRoot, SERVICE_ROOT) || ".";                   // 상대 경로 계산
  fs.mkdirSync(path.join(SERVICE_ROOT, "app"),   { recursive: true });                     // app/ 생성
  fs.mkdirSync(path.join(SERVICE_ROOT, "src"),   { recursive: true });                     // src/ 생성
  fs.mkdirSync(path.join(SERVICE_ROOT, "tests"), { recursive: true });                     // tests/ 생성
  fs.mkdirSync(path.join(SERVICE_ROOT, "docs"),  { recursive: true });                     // docs/ 생성
  const readmePath = path.join(SERVICE_ROOT, "README.md");                                 // README 경로
  if (!existsSync(readmePath)) {                                                           // README 없으면
    fs.writeFileSync(                                                                      // 초기 README 생성
      readmePath,                                                                          // 대상 파일
      `# Auto Service Root

- This directory is the sandbox for generated code.
- Create app/src/docs/tests under here.

`,                                                                                         // 내용(간단 안내)
      "utf8"                                                                               // 인코딩
    );                                                                                     // 쓰기 끝
  }

  function appendStage(stage, ok, details){                                                // stage 로그 기록 헬퍼
    let arr = [];                                                                          // 기존 배열 초기화
    try { arr = JSON.parse(fs.readFileSync(stageLogPath, "utf8")); } catch {}              // 기존 로드(없어도 OK)
    arr.push({ stage, ok: !!ok, details: details ?? null, ts: nowIso() });                 // 항목 추가
    fs.writeFileSync(stageLogPath, JSON.stringify(arr, null, 2), "utf8");                  // 파일 저장
  }
  function writeRunMeta(extra){                                                            // run 메타 병합 저장
    const base = {                                                                         // 기본 필드
      startedAt: nowIso(),                                                                 // 시작 시각
      event: evt.action || eventName || "n/a",                                             // 이벤트/액션
      source: isIssue ? "issue" : (isDispatch ? "dispatch" : "comment"),                   // 이벤트 소스
      sourceIssueNumber,                                                                   // 원 이슈 번호
      tokens: [],                                                                          // /auto 토큰 배열
      planOnly: false,                                                                     // 플랜 전용 여부
      longMode: false,                                                                     // 장기 모드 여부
      llm: null, model: null, agent: null, agentModel: null,                               // 라우팅 메타
      branch: null, prNumber: null,                                                        // Git 메타
      serviceRoot: REL_SERVICE_ROOT                                                        // 서비스 루트 표시
    };
    try {                                                                                  // 기존 메타가 있으면
      const prev = JSON.parse(fs.readFileSync(runMetaPath, "utf8"));                       // 읽어서
      fs.writeFileSync(runMetaPath, JSON.stringify({ ...prev, ...base, ...(extra||{}) }, null, 2), "utf8"); // 병합
    } catch {                                                                              // 없거나 파싱 실패 시
      fs.writeFileSync(runMetaPath, JSON.stringify({ ...base, ...(extra||{}) }, null, 2), "utf8");          // 신규 저장
    }
  }
  appendStage("scaffold:service-root", true, { serviceRoot: REL_SERVICE_ROOT });           // 스캐폴딩 기록
  appendStage("bootstrap", true, {                                                         // 부트스트랩 기록
    hasAuto: true,                                                                         // /auto 존재 여부
    hasRunLabel: isDispatch ? "n/a(dispatch)" : labels.includes(labelsCfg.run || "automation:run") // 라벨 상태
  });
  writeRunMeta();                                                                          // 메타 초기 저장

  function toRegexes(globs = []) {                                                         // 글롭 → 정규식 변환
    return globs.map(s => new RegExp(s.replace(/\*\*/g, ".*").replace(/\*/g, "[^/]*")));  // 간이 글롭 치환
  }
  function matchesAny(p, regs) { return regs.some(r => r.test(p)); }                       // 경로 매칭 도우미

  function stageForCommit() {                                                              // 커밋 스테이징 헬퍼
    try {
      execSync(`git add -A`, { stdio: "inherit" });                                        // 전체 스테이징
      execSync(`git restore --staged .github/auto || true`, { stdio: "inherit" });         // 산출물 제외(언스테이지)
    } catch (e) {
      console.log("stageForCommit: ignored error", e?.message || e);                       // 무시 가능한 에러 로그
    }
  }

  function getStagedPaths() {                                                              // 스테이징 목록 읽기
    const out = execSync(`git diff --name-only --cached`, { encoding: "utf8" }).trim();    // 캐시드 변경 파일
    return out ? out.split("\n") : [];                                                     // 배열 반환
  }

  function validateStagedPathsOrThrow(pol) {                                               // 정책 검증(스테이징 대상만)
    const allowedRegs   = toRegexes(pol.allowed_globs   || ["^src/","^app/","^docs/","^apps/","^services/","^README\\.md$"]); // 허용
    const forbiddenRegs = toRegexes(pol.forbidden_globs || ["^\\.env","^secrets/","^\\.git/"]); // 금지
    const files = getStagedPaths();                                                        // 스테이징 파일 목록

    const offenders = [];                                                                  // 위반 목록
    for (const f of files) {                                                               // 각 파일 확인
      if (matchesAny(f, forbiddenRegs)) { offenders.push({ path: f, reason: "forbidden" }); continue; } // 금지 경로
      if (!matchesAny(f, allowedRegs))  { offenders.push({ path: f, reason: "outside_allowed" }); }     // 허용 외
    }

    if (offenders.length) {                                                                // 위반 존재 시
      appendStage("guard:path-offenders", false, { offenders, scope: "staged-only" });     // 로그 기록
      throw new Error("Changes include paths outside policy. See stage-log offenders.");   // 예외로 중단
    }
    appendStage("guard:path-ok", true, { filesCount: files.length, files });               // 정상 기록
  }

  const { tokens, hooks: rawHooks } = await loadPlugins(pluginsPaths, repoRoot);           // 플러그인 로드
  const hooks = {                                                                          // 훅 기본 형태 병합
    beforeLLM: [], afterLLM: [], beforeAgent: [], afterAgent: [], beforePR: [], afterPR: [],
    ...(rawHooks || {}),
  };

  const after = rawBody.replace(/^\/auto\s*/i, "");                                        // /auto 접두부 제거
  const m = after.match(/^((?:[\w:-]+)\b\s*)+/i) || [""];                                  // 연속 토큰 시퀀스 추출
  const seq = (m[0] || "").trim().split(/\s+/).filter(Boolean);                           // 토큰 배열화
  const rest = after.slice((m[0]||"").length).trim();                                      // 나머지(사용자 요구)

  const ctx = {                                                                            // 실행 컨텍스트
    repoRoot, cfg, tools, policy, labels, pipeline,                                       // 설정/환경
    tokens: seq, tokenFlags: {},                                                          // 토큰·플래그 맵
    userDemand: rest,                                                                     // 사용자 요구 텍스트
    llm: null, model: null, agent: null, agentModel: null,                                // 라우팅 결과 슬롯
    agentPrompt: "",                                                                      // 에이전트 프롬프트
    planOnly: false,                                                                      // 플랜 전용 여부
    preferFast: false,                                                                    // (레거시) 빠른 모드
    preferFastAgent: null,                                                                // Agent 전용 빠른 모드(null=미지정)
    longMode: false, budgetMinutes: null, budgetSteps: null,                              // 루프 제어
    loopSummary: { startedAt: nowIso(), steps: [] },                                      // 루프 요약
    usageTotals: { openai: { input:0, output:0 }, gemini: { input:0, output:0 }, claude: { calls: 0, model: null } }, // ★ Claude 계측 추가
    diagnostics: { last: null },                                                          // 간단 진단
    prNumber: null, branch: null                                                          // Git 메타
  };
  writeRunMeta({ tokens: ctx.tokens });                                                   // 초기 메타에 토큰 기록

  for (const t of seq) {                                                                  // 토큰 시퀀스 처리
    const h = tokens.get(t.toLowerCase());                                                // 핸들러 조회
    if (h) await h(ctx);                                                                  // 존재 시 실행
  }

  const highCost = labels.includes(labelsCfg.highCost || "automation:high-cost");         // 고비용 라벨 확인
  const max = policy.hard_stop_chars_without_high_cost_label ?? 20000;                    // 입력 최대 길이
  const planThreshold = policy.plan_only_threshold_chars ?? 8000;                         // 플랜 전용 임계
  if (!(ctx.tokenFlags?.force && highCost)) {                                             // (force && highCost) 예외
    if (ctx.userDemand.length > max && !highCost) {                                       // 초과 시
      appendStage("guard:input-too-long", false, { len: ctx.userDemand.length });         // 로그 기록
      throw new Error("Input too long without high-cost label.");                         // 중단
    }
  }
  ctx.planOnly = ctx.planOnly || (ctx.userDemand.length > planThreshold && !highCost);    // 플랜 전용 판단
  appendStage("route:pre", true, { planOnly: ctx.planOnly });                             // 라우팅 전 로그

  // preferFastAgent 미지정(null)이면 preferFast 값을 따라가도록 폴백                                 // 분리 기본값
  const preferFastLLM = ctx.preferFast === true;                                          // LLM 빠른 모드 플래그
  const preferFastAgent = (ctx.preferFastAgent == null) ? ctx.preferFast : !!ctx.preferFastAgent; // Agent 빠른 모드

  const route = pickLLMAndAgent({                                                         // 라우터 호출(독립 프리셋)
    userDemand: ctx.userDemand,                                                           // 사용자 요구
    planOnly: ctx.planOnly,                                                               // 플랜 전용
    tools,                                                                                // 도구 설정
    preferFast: ctx.preferFast,                                                           // (레거시) fast
    preferFastLLM,                                                                        // LLM fast
    preferFastAgent                                                                       // Agent fast
  });
  ctx.llm = route.llm;                                                                    // LLM 공급자
  ctx.model = route.model;                                                                // LLM 모델
  ctx.agent = route.agent;                                                                // 에이전트 종류
  ctx.agentModel = route.agentModel || null;                                              // 에이전트 모델
  if (ctx.agent === "claude") { ctx.usageTotals.claude.model = ctx.agentModel || "(cli-default)"; } // ★ 보고용 모델 저장
  appendStage("route:selected", true, { llm: ctx.llm, model: ctx.model, agent: ctx.agent, agentModel: ctx.agentModel }); // 라우팅 로그
  writeRunMeta({ llm: ctx.llm, model: ctx.model, agent: ctx.agent, agentModel: ctx.agentModel, planOnly: ctx.planOnly }); // 메타 병합
  for (const h of (hooks.beforeLLM || [])) await h(ctx);                                   // LLM 전 훅 실행
  appendStage("hooks:beforeLLM", true, null);                                              // 훅 기록

  const systemGuard = [                                                                    // 가드레일(제약 안내)
    "[에이전트 가드레일]",                                                                // 섹션 제목
    `- 허용 경로: ${(policy.allowed_globs||["src/**","app/**","docs/**","apps/**","services/**","README.md"]).join(", ")}`, // 허용 경로
    `- 금지 경로: ${(policy.forbidden_globs||[".env*","secrets/**",".git/**"]).join(", ")}`, // 금지 경로
    `- 서비스 루트(신규 생성 루트): ${path.relative(repoRoot, SERVICE_ROOT)}/`,           // 생성 루트 고정
    "- 'app/', 'src/', 'docs/', 'tests/'는 서비스 루트 하위에 생성",                       // 구조 규칙
    "- 'packages/**'는 편집/생성 금지",                                                    // 툴킷 보호
    "- .env* / 비밀키 / .git 금지",                                                        // 보안
    "- 작은 커밋 단위 권장",                                                               // 커밋 가이드
    "- 쉘/Bash 명령 실행 금지(파일 편집/패치만)",                                          // 실행 금지
    "- 없는 디렉터리는 패치 내에서 생성 후 파일 추가"                                       // 생성 규칙
  ].join("\n");                                                                            // 문자열 결합

  const content = [                                                                        // LLM 입력 본문
    systemGuard, "\n[사용자 요구]", ctx.userDemand, "\n[원하는 산출물]",                   // 섹션 구성
    "- 변경 개요(목록)", "- 파일별 수정 계획", "- 안전 체크리스트",                         // 산출물 목록
    ctx.planOnly ? "- (플랜 전용: 실행명령 생략)" : "- 최종 실행할 수정 단계"               // 모드별 항목
  ].join("\n");                                                                            // 결합

  async function callOpenAIOnce({ userText }) {                                            // OpenAI 단발 폴백
    const fallbackModel = (ctx.tools?.openai?.default) || "gpt-4o-mini";                   // 폴백 모델
    const { text, usage } = await runOpenAI({                                              // 호출
      client: makeOpenAI(process.env.OPENAI_API_KEY),                                      // 클라이언트 생성
      model: fallbackModel,                                                                // 모델 지정
      system: systemGuard,                                                                 // 시스템 메시지
      user: userText,                                                                      // 사용자 입력
      reasoning: { effort: ctx.planOnly ? "medium" : "high" }                              // 리즈닝 강도
    });
    if (usage) {                                                                           // 사용량 존재 시
      ctx.usageTotals.openai.input += (usage.input_tokens||0);                             // 입력 토큰 합산
      ctx.usageTotals.openai.output += (usage.output_tokens||0);                           // 출력 토큰 합산
    }
    return text || "";                                                                     // 텍스트 반환
  }

  async function genPrompt(){                                                              // 프롬프트 생성 함수
    try {
      if (ctx.llm === "openai") {                                                          // OpenAI 경로
        const { text, usage } = await runOpenAI({                                          // 호출
          client: makeOpenAI(process.env.OPENAI_API_KEY),                                  // 클라이언트
          model: ctx.model, system: systemGuard, user: content,                            // 모델/본문
          reasoning: { effort: ctx.planOnly ? "medium" : "high" }                          // 리즈닝 강도
        });
        if (usage) {                                                                       // 사용량 반영
          ctx.usageTotals.openai.input += (usage.input_tokens||0);                         // 입력 합산
          ctx.usageTotals.openai.output += (usage.output_tokens||0);                       // 출력 합산
        }
        appendStage("llm:openai", true, { model: ctx.model });                              // LLM 로그
        return text || "";                                                                  // 결과 반환
      } else {                                                                              // Gemini 경로
        const { text } = await runGemini({                                                  // 호출
          client: makeGemini(process.env.GEMINI_API_KEY),                                   // 클라이언트
          model: ctx.model, user: content                                                   // 모델/본문
        });
        if (!text || !String(text).trim()) {                                               // 빈 응답 시
          appendStage("llm:gemini-empty", false, null);                                     // 로그
          const t = await callOpenAIOnce({ userText: content });                            // OpenAI 폴백
          appendStage("llm:fallback-openai", !!t, { used: "openai:single" });               // 폴백 로그
          return t || "";                                                                   // 반환
        }
        appendStage("llm:gemini", true, { model: ctx.model });                              // LLM 로그
        return text;                                                                        // 결과 반환
      }
    } catch (e) {                                                                           // 예외 처리
      appendStage("llm:error", false, { message: String(e?.message || e) });               // 에러 로그
      const t = await callOpenAIOnce({ userText: content });                                // OpenAI 폴백
      appendStage("llm:fallback-openai", !!t, { used: "openai:on-error" });                 // 폴백 로그
      return t || "";                                                                       // 결과 반환
    }
  }

  function synthesizePrompt() {                                                             // 프롬프트 폴백 합성
    return [
      "You are a senior full-stack engineer acting as an autonomous code agent.",           // 역할 정의
      "Follow these guardrails strictly:",                                                  // 규칙 안내
      systemGuard, "",                                                                      // 가드레일 삽입
      "[Task]", ctx.userDemand || "No explicit task text was provided. Propose a minimal safe change within allowed paths.", "", // 작업 정의
      "[Deliverables]", "- A short plan", "- File-by-file changes", "- Safety checklist", "- (If allowed) exact commands or edits" // 산출물 정의
    ].join("\n");                                                                           // 결합
  }

  function normalizeAgentPrompt(text) {                                                     // 프롬프트 정규화
    const trimmed = (text || "").trim();                                                    // 트림
    return trimmed || synthesizePrompt();                                                   // 비어있으면 합성본 사용
  }

  ctx.agentPrompt = normalizeAgentPrompt(await genPrompt());                                 // 프롬프트 확정
  appendStage("llm:prompt-ready", true, { length: ctx.agentPrompt.length });                 // 준비 로그
  for (const h of (hooks.afterLLM || [])) await h(ctx);                                      // LLM 후 훅
  appendStage("hooks:afterLLM", true, null);                                                 // 훅 로그

  if (ctx.tokenFlags?.dryRun) { writeRunMeta({ dryRun: true }); return { dryRun: true, ctx }; } // 드라이런 종료
  if (ctx.agent === "none" || ctx.planOnly) { writeRunMeta({ planOnly: ctx.planOnly }); return { planOnly: true, ctx }; } // 에이전트 스킵
  // Part 2/2에서 계속                                                                           // 파트 전환 주석
}
// packages/core/src/orchestrator.mjs                                                        // 파일 계속
// Part 2/2: 에이전트 실행·체크포인트·PR 생성·사용량 파일 저장                               // 파트 개요

// 아래 블록은 Part 1/2의 함수 스코프(runOrchestrator) 안에서 이어집니다.                    // 연결 지점

  function checkpointCommit(msg){                                                           // 체크포인트 커밋
    try {
      execSync(`git add -A`, { stdio: "inherit" });                                         // 전체 스테이징
      execSync(`git restore --staged .github/auto || true`, { stdio: "inherit" });          // 산출물 언스테이지
    } catch {}
    try {
      execSync(`git commit -m ${JSON.stringify(msg)}`, { stdio: "inherit" });               // 커밋 생성
    } catch {
      console.log("No changes to commit for checkpoint.");                                  // 변경 없음 안내
    }
  }

  execSync(`git config user.name "github-actions[bot]"`, { stdio: "inherit" });             // Git 사용자 이름
  execSync(`git config user.email "github-actions[bot]@users.noreply.github.com"`, { stdio: "inherit" }); // Git 이메일
  const branch = `auto/${Date.now()}`;                                                      // 고유 브랜치명(재보장)
  ctx.branch = branch;                                                                       // 컨텍스트 반영
  execSync(`git checkout -b ${branch}`, { stdio: "inherit" });                               // 새 브랜치 생성/체크아웃
  appendStage("git:branch", true, { branch });                                               // 단계 로그
  writeRunMeta({ branch });                                                                  // 메타 병합

  const cancelPath = path.join(outDir, "CANCEL");                                            // 취소 파일 경로
  const start = Date.now();                                                                  // 시작 시각 스냅샷

  async function runAgentWithFallback(promptText, toolsArg, policyArg) {                     // 에이전트 실행(폴백 포함)
    const primary = (ctx.agent || "").toLowerCase();                                         // 주 에이전트
    const fallback = primary === "claude" ? "cursor" : "claude";                             // 폴백 에이전트
    try {
      const safePrompt = normalizeAgentPrompt(promptText);                                   // 안전 프롬프트
      if (primary === "claude") {                                                            // Claude 사용 시
        ctx.usageTotals.claude.calls += 1;                                                   // ★ 호출수 +1
        await runWithClaude(safePrompt, toolsArg, policyArg, { model: ctx.agentModel });     // ★ 모델 명시 전달
      } else {                                                                               // 그 외(cursor)
        await runWithCursor(safePrompt, toolsArg, policyArg);                                 // Cursor 실행
      }
      appendStage("agent:primary", true, { used: primary, agentModel: ctx.agentModel || null }); // 성공 로그
      return { ok: true, used: primary };                                                    // 결과 반환
    } catch (e1) {                                                                            // 1차 실패
      appendStage("agent:primary", false, { used: primary, error: String(e1?.message||e1) }); // 실패 로그
      try {
        const safePrompt2 = normalizeAgentPrompt(promptText);                                 // 재시도 프롬프트
        if (fallback === "claude") {                                                          // Claude 폴백
          ctx.usageTotals.claude.calls += 1;                                                 // ★ 호출수 +1
          await runWithClaude(safePrompt2, toolsArg, policyArg, { model: ctx.agentModel });   // ★ 모델 명시 전달
        } else {                                                                              // Cursor 폴백
          await runWithCursor(safePrompt2, toolsArg, policyArg);                               // Cursor 실행
        }
        appendStage("agent:fallback", true, { chain: `${primary}->${fallback}` });            // 폴백 성공 로그
        return { ok: true, used: `${primary}->${fallback}` };                                 // 결과 반환
      } catch (e2) {                                                                          // 폴백 실패
        appendStage("agent:fallback", false, { chain: `${primary}->${fallback}`, error: String(e2?.message||e2) }); // 실패 로그
        return { ok: false, used: `${primary}->${fallback}` };                                // 결과 반환
      }
    }
  }

  async function runOneStep(step) {                                                          // 단일 스텝 실행
    for (const h of (hooks.beforeAgent || [])) await h(ctx);                                 // 에이전트 전 훅
    appendStage("hooks:beforeAgent", true, { step });                                        // 훅 로그

    const originalCwd = process.cwd();                                                       // 원 CWD 저장
    try {
      process.chdir(SERVICE_ROOT);                                                           // 서비스 루트로 이동
      appendStage("cwd:enter-service-root", true, { cwd: path.relative(repoRoot, SERVICE_ROOT) }); // CWD 로그
      const result = await runAgentWithFallback(ctx.agentPrompt, tools, policy);             // 에이전트 실행
      ctx.agentUsed = result.used || ctx.agent;                                              // 실제 사용 에이전트
      appendStage("hooks:afterAgent", true, { step, ok: result.ok });                        // 훅 로그

      process.chdir(repoRoot);                                                               // 저장소 루트 복귀

      stageForCommit();                                                                      // 스테이징 수행
      validateStagedPathsOrThrow(policy);                                                    // 정책 검증

      checkpointCommit(`auto: checkpoint step ${step}`);                                     // 체크포인트 커밋
      try { execSync(`git push origin ${branch}`, { stdio: "inherit" }); } catch {}          // 푸시(실패 무시)
      appendStage("git:push", true, { step });                                               // 푸시 로그
      ctx.loopSummary.steps.push({ step, at: nowIso(), agentUsed: result.used, ok: result.ok }); // 루프 요약
      writeRunMeta({ loopSummary: ctx.loopSummary });                                        // 메타 병합
      const diagFile = path.join(outDir, "diagnostics-last.json");                           // 진단 파일 경로
      writeFileSync(diagFile, JSON.stringify({ when: nowIso(), logs: [{ stage: "agent", ok: true, out: result.used }] }, null, 2), "utf8"); // 진단 저장
    } finally {
      try { process.chdir(originalCwd); appendStage("cwd:leave-service-root", true, { restored: true }); } catch {} // CWD 복구
    }
  }

  if (ctx.longMode) {                                                                        // 장기 모드
    const maxMs = (ctx.budgetMinutes||60) * 60 * 1000;                                       // 시간 한도(ms)
    const maxSteps = ctx.budgetSteps || 3;                                                   // 스텝 한도
    for (let step=1; step<=maxSteps; step++) {                                               // 스텝 반복
      if (existsSync(cancelPath)) { appendStage("long:cancel", true, null); break; }         // 취소 파일 감지
      if ((Date.now()-start) > maxMs) { appendStage("long:timeout", false, { maxMs }); break; } // 타임아웃
      await runOneStep(step);                                                                 // 실행
    }
  } else {
    await runOneStep(1);                                                                      // 단발 실행
  }

  for (const h of (hooks.beforePR || [])) await h(ctx);                                       // PR 전 훅
  appendStage("hooks:beforePR", true, null);                                                  // 훅 로그

  const agentUsed = (ctx.agentUsed && String(ctx.agentUsed).trim()) || ctx.agent;             // 실제 에이전트
  const tokenList = (ctx.tokens||[]).join(", ") || "(none)";                                  // /auto 토큰 목록
  const labelList = labels.join(", ") || "(none)";                                            // 라벨 목록
  const truncatedUserDemand = (ctx.userDemand || "").slice(0, 2000);                          // 요구 미리보기
  const costLine = `OpenAI usage: in=${ctx.usageTotals.openai.input} out=${ctx.usageTotals.openai.output}`; // OpenAI 사용량
  const claudeLine = `Claude usage: model=${ctx.usageTotals.claude.model || "(n/a)"} calls=${ctx.usageTotals.claude.calls}`; // ★ Claude 사용량
  const sourceIssueLine = sourceIssueNumber ? `Source-Issue: #${sourceIssueNumber}` : "Source-Issue: n/a"; // 소스 이슈 표기

  const infoMd = [                                                                            // PR 본문 조합
    `## Auto-run Info`,                                                                       // 제목
    ``,
    `- LLM (plan): **${ctx.llm}** (${ctx.model})`,                                            // LLM 정보
    `- Agent (requested): **${ctx.agent}**`,                                                  // 요청 에이전트
    `- Agent (actual): **${agentUsed}**`,                                                     // 실제 에이전트
    `- Agent Model: ${ctx.agentModel || "(n/a)"}`,                                            // 에이전트 모델
    `- ${claudeLine}`,                                                                        // ★ Claude 사용량 라인
    `- Branch: ${ctx.branch}`,                                                                // 브랜치명
    `- Service Root: ${path.relative(repoRoot, SERVICE_ROOT)}/`,                              // 서비스 루트
    `- Labels: ${labelList}`,                                                                 // 라벨
    `- Tokens: ${tokenList}`,                                                                 // /auto 토큰
    `- ${costLine}`,                                                                          // OpenAI 사용량
    `- ${sourceIssueLine}`,                                                                   // 소스 이슈
    ``,
    `## Prompt (truncated)`,                                                                  // 프롬프트 섹션
    "", "```", truncatedUserDemand, "```", ""                                                 // 코드블록
  ].join("\n");                                                                               // 결합

  const promptMd = [                                                                          // 컨텍스트 코멘트
    `# Original Prompt`, "", "```", ctx.userDemand, "```", "",                                // 원 프롬프트
    "## Last Agent Prompt", "", "```", (ctx.agentPrompt || "").trim(), "```"                  // 최종 프롬프트
  ].join("\n");                                                                               // 결합

  const prBodyPath = path.join(outDir, `pr-body-${RUN_ID}.md`);                               // PR 본문 파일
  const promptBodyPath = path.join(outDir, `prompt-${RUN_ID}.md`);                            // 프롬프트 파일
  writeFileSync(prBodyPath, infoMd, "utf8");                                                  // 파일 저장
  writeFileSync(promptBodyPath, promptMd, "utf8");                                            // 파일 저장

  const title = `auto: ${ctx.branch} [${ctx.llm}/${agentUsed}] (tokens: ${tokenList})`;       // PR 제목
  execSync(                                                                                   // PR 생성
    `gh pr create --title ${JSON.stringify(title)} --body-file ${JSON.stringify(prBodyPath)} --base main --head ${ctx.branch}`,
    { stdio: "inherit" }                                                                       // 서브프로세스 출력 계승
  );
  appendStage("pr:create", true, { title });                                                   // 생성 로그

  const prNumber = execSync(                                                                   // PR 번호 조회
    `gh pr list -s all --head ${ctx.branch} --json number --jq '.[0].number // empty'`
  ).toString().trim();                                                                         // 문자열 변환
  ctx.prNumber = prNumber || null;                                                             // 컨텍스트 반영
  writeRunMeta({ prNumber: ctx.prNumber });                                                    // 메타 병합
  appendStage("pr:number", !!ctx.prNumber, { prNumber: ctx.prNumber });                        // 단계 로그

  if (prNumber) {                                                                              // PR 번호가 있으면
    execSync(                                                                                  // 프롬프트 코멘트 추가
      `gh pr comment ${prNumber} --body-file ${JSON.stringify(promptBodyPath)}`,
      { stdio: "inherit" }
    );
    appendStage("pr:comment-prompt", true, { prNumber });                                      // 코멘트 로그

    const autoMerge = labelsCfg.autoMerge || "automation:auto-merge";                           // 자동 머지 라벨
    const cont = labelsCfg.continue || "automation:continue";                                   // 컨티뉴 라벨
    try {
      execSync(`gh pr edit ${prNumber} --add-label ${JSON.stringify(autoMerge)} --add-label ${JSON.stringify(cont)}`, { stdio: "inherit" }); // 라벨 부여
      appendStage("pr:labels", true, { added: [autoMerge, cont] });                             // 라벨 로그
    } catch (e) {
      appendStage("pr:labels", false, { error: String(e?.message || e) });                      // 실패 로그
    }
  }

  // 사용량 요약 파일 저장(런 단위)                                                            // 보고 산출물
  const agentUsagePath = path.join(outDir, "agent-usage.json");                                // 파일 경로
  writeFileSync(agentUsagePath, JSON.stringify({                                               // JSON 저장
    when: nowIso(),                                                                            // 기록 시각
    claude: { model: ctx.usageTotals.claude.model, calls: ctx.usageTotals.claude.calls },      // Claude 요약
    openai: ctx.usageTotals.openai,                                                            // OpenAI 토큰 요약
    gemini: ctx.usageTotals.gemini                                                             // Gemini 토큰 요약
  }, null, 2), "utf8");                                                                        // pretty 저장

  for (const h of (hooks.afterPR || [])) await h(ctx);                                          // PR 후 훅
  appendStage("hooks:afterPR", true, null);                                                     // 훅 로그

  writeRunMeta({ finishedAt: nowIso() });                                                       // 종료 시각 저장
  appendStage("done", true, { longMode: ctx.longMode });                                        // 완료 로그

  return { success: true, branch: ctx.branch, long: ctx.longMode, usage: ctx.usageTotals, prNumber: ctx.prNumber }; // 최종 리턴
}                                                                                               // runOrchestrator 끝
