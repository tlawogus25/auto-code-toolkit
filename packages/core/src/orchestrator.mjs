...
export async function runOrchestrator({ repoRoot, configPath, eventPath }) {
  ...
  async function genPrompt(){
    if (ctx.llm === "openai") {
      ...
      const { text } = await runGemini({ client: makeGemini(process.env.GEMINI_API_KEY), model: ctx.model, user: content });
      return text;
    }
  }
  ...
  const cancelPath = path.join(repoRoot, ".github/auto/CANCEL");
  ...
  if (ctx.longMode) {
    ...
    for (let step=1; step<=maxSteps; step++) {
      ...
      if (!ctx.agentPrompt || typeof ctx.agentPrompt !== "string") ctx.agentPrompt = "Continue.";
    }
  } else {
    await runOneStep(1);
  }
  ...
  const prBodyPath = path.join(repoRoot, `.github/auto/pr-body-${Date.now()}.md`);
  const promptBodyPath = path.join(repoRoot, `.github/auto/prompt-${Date.now()}.md`);
  ...
  const title = `auto: ${branch} [${ctx.llm}/${ctx.agent}] (tokens: ${tokenList})`;
  execSync(`gh pr create --title ${JSON.stringify(title)} --body-file ${JSON.stringify(prBodyPath)} --base main --head ${branch}`, { stdio: "inherit" });

  const prNumber = execSync(`gh pr view --json number --head ${branch} --jq .number`).toString().trim();
  ctx.prNumber = prNumber || null;

  if (prNumber) {
    execSync(`gh pr comment ${prNumber} --body-file ${JSON.stringify(promptBodyPath)}`, { stdio: "inherit" });
  }

  for (const h of hooks.afterPR) await h(ctx);

  return { success: true, branch, long: ctx.longMode, usage: ctx.usageTotals, prNumber: ctx.prNumber };
}
